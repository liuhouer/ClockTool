{===============================================================================
  ...
  ------------------------------------------------------------------------------
  TRzSplitter           Container component with two panes.  The size of each
                          pane is controlled by the splitter bar.
  TRzSizePanel          Enhanced panel that can be resized at runtime using a
                          sizing bar.
...
procedure CreateMaskBitmap( MaskBmp: TBitmap );
...
begin
  ...
  begin
    Brush.Color := clWhite;
    FillRect( Rect( 0, 0, MaskBmp.Width, MaskBmp.Height ) );
    for I := 0 to 7 do
      for J := 0 to 7 do
        if ( I + J ) mod 2 <> 0 then
          Pixels[ I, J ] := clBlack;
...
procedure TRzSplitter.SetUseGradients( Value: Boolean );
begin
...
begin
  ...
  try
    ...
    try
      ...
      Canvas.Brush.Bitmap := FMaskBmp;
      PatBlt( Canvas.Handle, P.X, P.Y,
...
begin
  R := DrawSides( Canvas, FHotSpotRect, clBtnShadow, clBtnShadow, sdAllSides );
  ...
  begin
    ...
    DrawSides( Canvas, R, clBtnHighlight, clNone, [ sdLeft, sdTop ] );
    ...
    begin
      ...
      Canvas.MoveTo( MidX - 1, FHotSpotRect.Top + 1 );
      Canvas.LineTo( MidX - 1, FHotSpotRect.Bottom - 1 );
    end;
    ...
    begin
      ...
      begin
        Canvas.Pixels[ FHotSpotRect.Left + 15 + I * 3, MidY ] := clHighlightText;
        Canvas.Pixels[ FHotSpotRect.Left + 16 + I * 3, MidY + 1 ] := clHighlight;
      end;
    end;
  end
  else
  begin
    ...
    begin
      ...
      Canvas.Pen.Color := clBtnHighlight;
      Canvas.MoveTo( FHotSpotRect.Left + 1, MidY - 1 );
      Canvas.LineTo( FHotSpotRect.Right - 1, MidY - 1 );
    end;
    ...
    begin
      ...
      begin
        Canvas.Pixels[ MidX, FHotSpotRect.Top + 15 + I * 3 ] := clHighlightText;
        Canvas.Pixels[ MidX + 1, FHotSpotRect.Top + 16 + I * 3 ] := clHighlight;
      end;
    end;
  end;
...
begin
  ...
  else if FUseGradients and ( FSplitterStyle = ssGroupBar ) then
    DrawGroupBarBackground( Canvas, FBarRect, FThemeAware, FGradientPath, Color, FGradientColorAdjustment );
    ...
    begin
      ...
      begin
        case Controls[ I ].Align of
          alTop:
            Inc( Result.Top, Controls[ I ].Height );

          alBottom:
            Dec( Result.Bottom, Controls[ I ].Height );

          alLeft:
            Inc( Result.Left, Controls[ I ].Width );

          alRight:
            Dec( Result.Right, Controls[ I ].Width );
        end;
      end;
    end;
  end
...
begin
  ...
  begin
    ...
    FillRect( R );
    DrawSides( Canvas, R, clBtnHighlight, clLime, [ sdLeft, sdTop ] );
    ...
    begin
      ...
      if ( ( Align = alTop ) and not FHotSpotClosed ) or
         ( ( Align = alBottom ) and FHotSpotClosed ) then
      begin
        { Draw /\ Arrows }
    ...
    begin
      ...
      if ( ( Align = alLeft ) and not FHotSpotClosed ) or
         ( ( Align = alRight ) and FHotSpotClosed ) then
      begin
        { Draw < Arrows }
...
begin
  ...
  FillColor := Color;
  if FUseGradients and ( FSizeBarStyle = ssGroupBar ) then
  begin
    if Align = alLeft then
      Dec( BarRect.Left )
  ...
  else if FUseGradients and ( FSizeBarStyle = ssGroupBar ) and ( Align in [ alLeft, alRight ] ) then
    DrawGroupBarBackground( Canvas, BarRect, FThemeAware, FGradientPath, Color, FGradientColorAdjustment );
...
procedure TRzCustomSizePanel.SetUseGradients( Value: Boolean );
begin
  if FUseGradients <> Value then
  begin
    FUseGradients := Value;
