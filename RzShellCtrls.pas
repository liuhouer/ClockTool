{===============================================================================
  ...
  TRzShellList          Explorer-like list view of folder contents.
  TRzShellCombo         Explorer-like combobox of the namespace.
...
type
  ...
  TRzShPopupHintEvent = procedure( Sender: TObject; const Hint: string ) of object;
   ...
   {The TRzShellLocator class is effectively a Variant type for shell object locations.
    ...
    Navigating to your particular folder at design time and storing as a PIDL is not sufficient. Remember    that a PIDL is just a binary pathname. For example, if you selected C:\WINDOWS\START MENU\ as a PIDL
    on another machine the Start Menu might be at E:\WIN95\START MENU\ and your code will break.
  ...
  protected
    ...
    property Options: TRzShellListOptions
      ...
      default [ sloAutofill, sloNonFilesystemAncestors, sloDefaultKeyHandling, sloContextMenus, sloDynamicRefresh,
                sloOleDrag, sloOleDrop, sloFolderContextMenu, sloShowHidden ];
...
begin
  ...
  try
    ...
    begin
      ...
      if Failed( Result ) then
        Exit;
        // This convoluted way of getting a pidl ensures we get the pidl to the real desktop folder
        // (eg. c:\windows\desktop) rather than the root 'desktop' folder. By getting c:\windows\desktop we can
        // use the IShellFolder of c:\windows to do things with 'desktop' that can't be done with the desktop
        // in its 'root of the namespace' manifestation.
      pa.Free;
...
procedure DoAutoScroll( ctl: TWinControl; code, direction: Integer );
const
...
begin
  Result := SFGAO_HASSUBFOLDER or SFGAO_FOLDER or SFGAO_DISPLAYATTRMASK or
    ...
    SFGAO_CANCOPY or SFGAO_CANMOVE or SFGAO_CANLINK or SFGAO_CANDELETE; // or
//            SFGAO_VALIDATE;
...
begin
  Result := ( Attributes and SFGAO_FOLDER ) <> 0;
end;
...
begin
  ...
  begin
    if SHGetFileInfo( PChar( AbsoluteIdList ), 0, shinf, SizeOf( TSHFileInfo ), SHGFI_PIDL or SHGFI_DISPLAYNAME ) <> 0 then
      SetString( FDisplayName, shinf.szDisplayName, StrLen( shinf.szDisplayName ) );
  end;
  Result := FDisplayName;
end;                                        {TRzShellListData.GetDisplayName}
...
begin
  ...
  attrib := ( SFGAO_HASSUBFOLDER or SFGAO_FOLDER or SFGAO_DISPLAYATTRMASK or SFGAO_FILESYSANCESTOR or
              SFGAO_FILESYSTEM or SFGAO_FILESYSANCESTOR or SFGAO_REMOVABLE or SFGAO_GHOSTED {or
              SFGAO_VALIDATE})
              and
              not SFGAO_READONLY;
  ...
  if fInclude then
  begin
    newnode := Items.AddChild( ParentNode, GetFriendlyName( Parent, RelIdList, SHGDN_INFOLDER ) );
...
begin {= TRzCustomShellList.OpenItem =}
  ...
  begin
    ...
    try
      ...
      tmppidl := ld.RelativeIdList;
      dw := FIShf.GetAttributesOf( 1, tmppidl, dwAttrib );
      if ( dw <> S_OK ) or ( ( dwAttrib and SFGAO_FOLDER ) = 0 ) then
...
begin
  ...
  try
    OleCheck( _IShellFolder.GetAttributesOf( c, PPItemIdList( pidla.List )^, PDWORD( dwa.List )^ ) );
    Result := DRAGFLAGS;
    for I := 0 to dwa.Count - 1 do
      Result := Result and Integer( dwa[ I ] );
  finally
...
begin
  ...
  begin
    ...
    try
      if Assigned( FShellTree ) then
        FShellTree.DoSetSelectedIdList( Folder.IdList )
      else if Assigned( FShellCombo ) then
        FShellCombo.SelectedFolder.IdList := Folder.IdList;
      FillItems;
    finally
      Dec( FIgnoreChanges );
    end;
...
const
  _DIR: array[ Boolean ] of TRzLVSortDirection = ( sdAscending, sdDescending );
begin
...
begin
  ...
  if ( sloHideFoldersWhenLinkedToTree in Options ) and
    ...
    ( ( attrib and ( SFGAO_FOLDER or SFGAO_FILESYSTEM ) = ( SFGAO_FOLDER or SFGAO_FILESYSTEM ) ) ) then
    Result := False;
...
begin
  ...
  if Assigned( FFilterLookupTable ) then
    Result := ( ( itemAttributes and SFGAO_FOLDER ) <> 0 ) or
...
begin
  Result := ( ( Attrib and SFGAO_FOLDER ) <> 0 ) or FilterLookup( FFilterLookupTable, FileName );
end;
...
begin
  ...
  begin
    ...
    try
      if Assigned( FShellTree ) then
        FShellTree.SelectedFolder := SelectedFolder
//        FShellTree.DoSetSelectedIdList( SelectedFolder.IdList )
      else if Assigned( FShellList ) then
        FShellList.Folder.IdList := SelectedFolder.IdList;
      FillItems;
    finally
      Dec( FIgnoreChanges )
    end;
    ...
    begin
      ...
      begin
        ...
        if dw = 0 then
          ...
          newitem := AddItem( GetFriendlyName( aIShf, curRelPidl, SHGDN_INFOLDER ),
