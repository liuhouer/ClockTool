{===============================================================================
  ...
  ------------------------------------------------------------------------------
  TRzPathBar            Navigational control that manages a set of path items
                          that can be used to create a "you are here" path --
                          sometimes referred to as a "breadcrumb path".
    ...
    begin
      ...
      begin
        if FItems[ I ].HighlightColor = clNone then
          ...
          C := FItems[ I ].HighlightColor;

        Canvas.Pen.Color := C;
        if FItems[ I ].CaptionState = csHot then
          Canvas.Brush.Color := LighterColor( C, 80 )
        else
          Canvas.Brush.Color := LighterColor( C, 60 );
...
procedure TRzPathBar.MouseUp( Button: TMouseButton; Shift: TShiftState; X, Y: Integer );
var
  DoClick: Boolean;
  I: Integer;
  P: TPoint;
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        begin
          FItems[ I ].ClickingCaption := False;
          DoClick := PtInRect( FItems[ I ].HotRect, Point( X, Y ) );
...
end; {= TRzPathBar.MouseUp =}


procedure TRzPathBar.MouseEnter;
begin
  inherited;
  Refresh;
end;
...
procedure TRzPathBar.MouseLeave;
var
  I: Integer;
begin
  FCaptionState := csNormal;
