{===============================================================================
  ...
  ------------------------------------------------------------------------------
  TRzLabel              Enhanced label control that supports 3D text styles,
                          angle rotation, fly-by colors, etc.
  TRzURLLabel           TRzLabel descendant that supports linking to a URL.
...
begin
  ...
  begin
    ...
    DC := GetDC( 0 );
    Canvas.Handle := DC;
    DoDrawText( R, dt_ExpandTabs or dt_CalcRect or WordWraps[ WordWrap ] or CondenseCaptionFlags[ FCondenseCaption ] );
...
begin
  ...
  begin
    ...
    DrawText( Canvas.Handle, CaptionStz, -1, CalcRect, Flags or dt_CalcRect or CondenseCaptionFlags[ FCondenseCaption ] );

    if Layout = tlBottom then
      OffsetRect( R, 0, Height - FBorderWidth - CalcRect.Bottom )
    else
      OffsetRect( R, 0, ( Height - FBorderWidth - CalcRect.Bottom ) div 2 );
  end;
...
begin
  ...
  Flags := dt_ExpandTabs or DrawTextAlignments[ Alignment ];
  if UseRightToLeftAlignment then
